...
DecodeClass::DecodeClass(std::istream &stream, uint16_t width, uint16_t height, Palette palette) :
...
int DecodeClass::decode80(uint8_t *image_out, uint32_t checksum)
{
...
int DecodeClass::decode40(const uint8_t *image_in, uint8_t *image_out)
{
    /*
       0 fill 00000000 c v
       1 copy 0ccccccc
...
void DecodeClass::decode2(std::istream &stream, uint8_t *out, int size)
{
    int count;
    while (size > 0) {
...
void DecodeClass::decode2(const std::vector<uint8_t> &in, uint8_t *out)
{
...
void DecodeClass::encode2(const uint8_t *source, int len, int slices, std::ostream &dest) {
    int count;
